...
stdenv.mkDerivation (
  finalAttrs:
  let
    jemallocStatic = jemalloc.overrideAttrs (old: {
      configureFlags = old.configureFlags ++ [ "--disable-cxx" ];
    });
  in
  {
    pname = "tmux";
    ...
    ++ lib.optionals withJemalloc [ jemallocStatic ];
    ...
    ++ lib.optionals withJemalloc [ "--enable-jemalloc" ];

    enableParallelBuilding = true;

    postConfigure = lib.optionalString (stdenv.hostPlatform.isDarwin && withJemalloc) ''
      # Force load jemalloc to override system malloc, mimicking the static build script
      sed -i "s|-ljemalloc|-Wl,-force_load,${jemallocStatic}/lib/libjemalloc.a|g" Makefile
    '';
